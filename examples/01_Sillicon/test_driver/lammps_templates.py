...
class LammpsTemplates:
    def __init__(self, root):
        ...
        fix record all print 10 "$(step) $(vol) $(temp) $(lx) $(ly) $(lz) $(xy) $(xz) $(yz)" &
                           title "step | vol | temp | lx | ly | lz | xy | xz | yz" &
                           screen no &
                           file output/equilibration.dat

        # Set up convergence check with kim-convergence.
        python run_length_control input 18 SELF 1 variable vol_metal variable temp_metal variable lx_metal variable ly_metal variable lz_metal variable xy_metal variable xz_metal variable yz_metal format pissssssssssssssss file test_driver/run_length_control_preFL.py
        ...
        # Compute mean squared displacement of each atom type
